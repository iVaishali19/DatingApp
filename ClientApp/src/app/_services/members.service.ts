...
//   currentUser$ = this.currentUserSource.asObservable();
  ...
  getMembers(userParams: UserParams){
    ...
    //if(this.member.length>0) return of(this.member);
      ...
      params=params.append('minAge',userParams.maxAge.toString());
      params=params.append('minAge',userParams.gender);
  ...
  getMember(username :string){    
    const member=  [...this.memberCache.values()].reduce((arr,elem)=>
      arr.concat(elem.result),[]).find((member:Member)=>member.userName== username);
